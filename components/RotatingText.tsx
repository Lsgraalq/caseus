...
export default function RotatingText({ text }: { text: string }) {
  ...
  const items = [...Array(4)].map((_, i) => (
    <h1 key={i} className="text-[60px] md:text-[130px] apercu-bold uppercase px-8 shrink-0">
      {text}
    </h1>
  ));

  return (
    <div className="w-full h-[100px] md:h-[180px] bg-white text-black flex items-center overflow-hidden">
