...
export default function Home() {
  ...
  // 2. ФЕТЧИМ ПРОЕКТЫ ИЗ SANITY ПРИ ЗАГРУЗКЕ
  useEffect(() => {
    const fetchProjects = async () => {
      // Берем 3 самых новых проекта
  ...
  // 3. ОТДЕЛЬНЫЙ USE-EFFECT ДЛЯ АНИМАЦИИ ПРОЕКТОВ (ждет загрузки данных)
  useEffect(() => {
    if (projects.length > 0) {
      ...
      // Обновляем триггеры, так как мы добавили новые элементы в DOM
  ...
  return (
    <>
      <main className="relative z-10 bg-white mb-[100vh] rounded-3x;">
        ...
        {/* PROJECT SECTION - ТЕПЕРЬ ДИНАМИЧЕСКАЯ */}
